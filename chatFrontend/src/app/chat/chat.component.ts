...
export class ChatComponent implements OnInit {
  ...
  loadChatByEmail(event: string, event1: string) {
    ...
    // For checking the chat room by both the emails,
    // if there is present then it will give the chat Id in sessionStorage
    this.chatService.getChatByFirstUserNameAndSecondUserName(event, event1).subscribe(data => {
      ...
      if (this.chatData && this.chatData.length > 0) {
        // Check if chatData is not null and has at least one element
        this.chatId = this.chatData[0].chatId;
        console.log(this.chatId);
        sessionStorage.setItem('chatId', this.chatId);
  
        // Set interval to get chat details
        setInterval(() => {
  ...
  goToChat(username: any) {
    ...
    this.chatService.getChatByFirstUserNameAndSecondUserName(username, sessionStorage.getItem("username")).subscribe(
      ...
      if (usernameFromSession !== null) {
        this.chatObj.firstUserName = usernameFromSession;
      }
      },
      (error) => {
        if (error.status == 404) {
